...
FROM 
    (SELECT a.account_id, sum(t.amount) - a.max_income AS diff,month(day) as day,
     extract(month FROM lead(t.day) over(PARTITION BY a.account_id ORDER BY day)) next
      FROM Transactions t
      JOIN Accounts a ON t.account_id = a.account_id
      WHERE t.type = 'Creditor'
      GROUP BY t.account_id,month(day)
      HAVING diff > 0
      ORDER BY t.transaction_id) a
WHERE day + 1=next